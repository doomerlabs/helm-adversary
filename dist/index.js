#!/usr/bin/env node
import { createRequire as __helmCreateRequire } from 'node:module'; const require = __helmCreateRequire(import.meta.url);
...
var POD_TEMPLATE_KINDS = /* @__PURE__ */ new Set([
  "CronJob",
  "DaemonSet",
  "Deployment",
  "Job",
...
function evaluate(rule, sources, allPaths) {
  ...
  if (match.kind === "indented-block-content") {
    return matchingSources.flatMap(
      (file) => extractIndentedBlocks(file.source, match.blockStart).flatMap((block) => {
...
function findConditionalFileMounts(rule, file) {
  ...
  for (const document of helmDocuments(file.source)) {
    ...
    if (!isPodBearingDocument(guardedLines)) continue;
    for (let index = 0; index < guardedLines.length; index += 1) {
      const containerHeader = guardedLines[index];
      if (containerHeader === void 0 || !containerHeader.activeYaml || !/^(?:containers|initContainers|ephemeralContainers):$/.test(containerHeader.source.trim())) {
        continue;
      }
      ...
      for (const container of findContainerScopes(guardedLines, index)) {
        ...
        for (const argument of args) {
          const detection = detectConditionalFileMount(
            rule,
            file,
            container,
            argument,
          ...
          if (detection !== void 0) detections.push(detection);
        }
      }
    }
  }
...
function detectConditionalFileMount(rule, file, container, argument, mounts, volumes) {
  if (!argument.provable) return void 0;
  const matchingMounts = mounts.filter(
    (mount) => mount.provable && (argument.path === mount.mountPath || argument.path.startsWith(`${mount.mountPath}/`))
  );
  if (matchingMounts.length === 0) return void 0;
  const fullyAvailable = matchingMounts.some((mount) => {
    if (missingAvailabilityPredicate(argument, mount) !== void 0) return false;
    return volumes.some(
      (volume) => volume.provable && volume.name === mount.name && missingAvailabilityPredicate(argument, volume) === void 0
    );
  });
  if (fullyAvailable) return void 0;
  for (const mount of matchingMounts) {
    ...
    const volumeMismatch = matchingVolumes.map((volume) => ({ volume, mismatch: missingAvailabilityPredicate(argument, volume) })).find(({ mismatch: mismatch2 }) => mismatch2 !== void 0);
    if (mountMismatch === void 0 && volumeMismatch === void 0) continue;
    const mismatch = mountMismatch ?? volumeMismatch?.mismatch;
    const resourceLine = mountMismatch === void 0 ? volumeMismatch?.volume.line ?? mount.line : mount.line;
    const guardLine = mismatch === void 0 ? void 0 : mount.predicates.get(mismatch) ?? volumeMismatch?.volume.predicates.get(mismatch);
    const anchor = [argument.line, resourceLine, guardLine].find((line) => line !== void 0 && isEligibleLine(file, line));
...
function annotateHelmGuards(source, startLine) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const trimmed = line.trim();
    if (blockScalarIndent !== void 0 && trimmed !== "" && indentation(line) <= blockScalarIndent) {
      blockScalarIndent = void 0;
    }
    const activeYaml = blockScalarIndent === void 0;
    ...
    for (const action of actions) {
      ...
      if (/^(?:range|define|block)\b/.test(expression)) {
        stack.push({ predicates: /* @__PURE__ */ new Map(), provable: false });
        continue;
      }
      if (/^else\b/.test(expression)) {
        if (stack.length > 0) stack[stack.length - 1] = { predicates: /* @__PURE__ */ new Map(), provable: false };
        continue;
      }
...
function parseGuard(expression, line, variables) {
  ...
  if (tokens.length === 1 && tokens[0]?.startsWith("$")) {
    return variables.get(tokens[0]) ?? { predicates: /* @__PURE__ */ new Map(), provable: false };
  }
  ...
  if (valueTokens.length === 0 || valueTokens.some((token) => !/^\.Values(?:\.[A-Za-z_]\w*)+$/.test(token))) {
    return { predicates: /* @__PURE__ */ new Map(), provable: false };
  }
  return {
...
function findFileArguments(container) {
  ...
  for (let index = 0; index < container.lines.length; index += 1) {
    ...
    if (section === void 0 || !section.activeYaml || indentation(section.source) !== container.propertyIndent || !/^(?:args|command):$/.test(section.source.trim())) {
      continue;
    }
    for (const item of yamlListItems(container.lines, index)) {
      const scalar = item.lines[0];
      if (scalar === void 0 || !scalar.activeYaml || scalar.source.trim().startsWith("#")) {
        continue;
      }
      const path = scalar.source.match(expression)?.[1];
      if (path === void 0 || path === "/" || path.includes("//")) continue;
...
function findVolumeMounts(lines, sectionIndent) {
  return findNamedYamlEntries(lines, "volumeMounts", sectionIndent).flatMap((entry) => {
    const mountPath = entry.lines.map((line) => ({ line, match: line.source.match(/^\s*(?:-\s*)?mountPath:\s*["']?(\/[A-Za-z0-9._/-]+)["']?\s*(?:#.*)?$/) })).find(
      ({ line, match }) => match !== null && (line === entry.lines[0] || indentation(line.source) === entry.propertyIndent)
    );
    if (mountPath?.match?.[1] === void 0) return [];
    return [{
...
function findNamedYamlEntries(lines, sectionName, expectedIndent) {
  ...
  for (let sectionIndex = 0; sectionIndex < lines.length; sectionIndex += 1) {
    ...
    if (section === void 0 || !section.activeYaml || section.source.trim() !== `${sectionName}:` || expectedIndent !== void 0 && indentation(section.source) !== expectedIndent) {
      continue;
    }
    for (const candidate of yamlListItems(lines, sectionIndex)) {
      ...
      const named = candidate.lines.map((line) => ({
        line,
        name: line.source.match(/^\s*(?:-\s*)?name:\s*["']?([A-Za-z0-9._-]+)["']?\s*(?:#.*)?$/)?.[1]
      })).find(
        ({ line, name }) => name !== void 0 && (line === first || indentation(line.source) === propertyIndent)
      );
      if (named?.name === void 0) continue;
      entries.push({
        section,
        name: named.name,
        nameLine: named.line,
...
function findContainerScopes(lines, sectionIndex) {
  return yamlListItems(lines, sectionIndex).map((item) => {
    ...
    const named = item.lines.map((line) => ({
      line,
      name: line.source.match(/^\s*(?:-\s*)?name:\s*["']?([A-Za-z0-9._-]+)["']?\s*(?:#.*)?$/)?.[1]
    })).find(
      ({ line, name }) => name !== void 0 && (line === item.lines[0] || indentation(line.source) === propertyIndent)
    );
    return {
...
function yamlListItems(lines, sectionIndex) {
  ...
  for (let index = sectionIndex + 1; index < lines.length; index += 1) {
    ...
    if (!line.activeYaml || trimmed === "" || trimmed.startsWith("#")) continue;
    const lineIndent = indentation(line.source);
    if (lineIndent <= sectionIndent) break;
    if (/^\s*-\s+/.test(line.source) && (itemIndent === void 0 || lineIndent === itemIndent)) {
...
function enclosingYamlBlock(lines, childIndex, childIndent) {
  ...
  for (let index = childIndex - 1; index >= 0; index -= 1) {
    const line = lines[index];
    if (line === void 0 || !line.activeYaml || line.source.trim().startsWith("#")) {
      continue;
    }
    const lineIndent = indentation(line.source);
...
function isPodBearingDocument(lines) {
  return lines.some((line) => {
    if (!line.activeYaml || indentation(line.source) !== 0) return false;
    const kind = line.source.match(/^kind:\s*["']?([A-Za-z][A-Za-z0-9]*)["']?\s*(?:#.*)?$/)?.[1];
    return kind !== void 0 && POD_TEMPLATE_KINDS.has(kind);
  });
}
...
function locateEligible(file, expression, offset = 0, source = file.source, anchors) {
  ...
  for (const match of source.matchAll(matcher)) {
    ...
    const anchor = anchors === void 0 ? eligibleAnchor(file, startLine, endLine) : eligibleSemanticAnchor(file, match[0], start, anchors);
    if (anchor === void 0) continue;
...
function extractIndentedBlocks(source, start) {
  ...
  while ((match = expression.exec(source)) !== null) {
    ...
    const indentation2 = source.slice(lineStart, firstLineEnd).match(/^[ \t]*/)?.[0].length ?? 0;
    ...
    while (cursor < source.length) {
      ...
      if (line.trim() !== "") {
        ...
        if (currentIndentation <= indentation2) {
          end = cursor;
          break;
        }
      }
